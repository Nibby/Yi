...
public final class AppBoardThemeSettings extends AppSettingsModule {
    ...
    private void loadThemeSettings(String selectedThemeDirectory) {
        final Path customThemeDir = AppSettings.getRootPath()
                .resolve(AppSettings.THEME_DIRECTORY_NAME).resolve("board");
    ...
    private void loadCustomTheme(Path customThemeDir, String selectedThemeDirectory) {
        ...
        try {
            AppSettings.readJSON(jsonFile).ifPresent(settingsJson -> this.loadFromJson(themeDir, settingsJson, true));
        } catch (IOException e) {
            // TODO: Error handling
            e.printStackTrace();
        }
    ...
    private Optional<Image> loadOptionalImage(boolean isCustomTheme, String themeDirectory,
        ...
        if (themeSettings.has(key)) {
            ...
            if (isCustomTheme) {
                ...
                inputStream = AppBoardThemeSettings.class.getResourceAsStream(imagePathName);
            }
    ...
    public String getSelectedThemeDirectory() {
        return AppSettings.general.getSelectedBoardTheme();
    }
