 ...
 */
public abstract class AppAbstractAction<M extends MenuItem, C extends Node> implements AppAction {

    private final Property<TextResource> nameProperty = new Property<>(AppText.EMPTY);
    ...
    private AppAction parentAction = null;

    private AppActionContext context;
    private Runnable action;

    private boolean addedToMenu = false;
    private AppMainMenuType mainMenuType = null;
    ...
    private final NullableProperty<AppAccelerator> acceleratorId = new NullableProperty<>(null);
     ...
     */
    public AppAbstractAction(@NotNull TextResource name) {
        this(name, null);
    }
    ...
    private void updateNodeCompactness(Labeled node, boolean isCompact) {
        if (isCompact) {
            ...
            acceleratorId.get().ifPresent(id -> {
                var accelerator = AppAccelerator.getAccelerator(id);
    ...
    private void installMenuItemAccelerator(AppAccelerator accelerator, MenuItem menuItem) {
        if (GlobalHelper.isRunningAsTest() && (accelerator == null || menuItem == null)) {
        ...
        var id = accelerator.getId();
        Accelerator.install(id, menuItem);
    }

    @Override
    public void setContext(AppActionContext context) {
     ...
     */
    protected final @NotNull AppActionContext getContext() {
        return Objects.requireNonNull(context, "Context not yet initialized");
    }

    @Override
    public void setAction(Consumer<AppActionContext> action) {
        if (action != null) {
            this.action = () -> {
    ...
    @Override
    public AppAction setInMenuBar(@NotNull AppMainMenuType menu, double position) {
    ...
    @Override
    public @NotNull AppMainMenuType getMainMenuType() {
    ...
    @Override
    public AppAction setText(@NotNull TextResource i18nKey) {
        this.nameProperty.set(i18nKey);
        return this;
    }

    @Override
    public AppAction setIcon(@Nullable ImageView icon) {
        var iconToUse = icon;
    ...
    @Override
    public AppAction setVisible(boolean visible) {
    ...
    protected final void setParentAction(AppSubMenuAction action) {
