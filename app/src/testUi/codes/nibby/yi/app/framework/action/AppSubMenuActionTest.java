 ...
 */
public class AppSubMenuActionTest extends AppUITestBase {
    ...
    private AppSubMenuAction subMenu1;
    private AppAction itemInSubMenu1;
    private final AtomicBoolean itemInSubMenu1ActionExecuted = new AtomicBoolean(false);

    private AppSubMenuAction subMenu2;
    private AppAction itemInSubMenu2;
    ...
    protected void initializeTestActions(AppActionManager actionManager) {
        subMenu1 = new AppSubMenuAction(AppText.MENUITEM_NEW_GAME);
        subMenu1.setInMenuBar(AppMainMenuType.TESTING, 0d);

        itemInSubMenu1 = new AppBasicAction(AppText.MENU_PERSPECTIVE,
                context -> itemInSubMenu1ActionExecuted.set(true));
        itemInSubMenu1.setInMenuBar(AppMainMenuType.TESTING, 0d);
        itemInSubMenu1.setAccelerator(AppAccelerator.TEST_ACCEL_1);

        subMenu1.addChildAction(itemInSubMenu1);

        subMenu2 = new AppSubMenuAction(AppText.MENUITEM_SAVE_AS_GAME);
        subMenu2.setInMenuBar(AppMainMenuType.TESTING, 1d);

        itemInSubMenu2 = new AppBasicAction(AppText.MENUITEM_SAVE_GAME,
                context -> itemInSubMenu2ActionExecuted.set(true));
        itemInSubMenu2.setInMenuBar(AppMainMenuType.TESTING, 1d);
        itemInSubMenu2.setAccelerator(AppAccelerator.TEST_ACCEL_2);
        ...
        actionManager.addActions(new AppAction[] {
                subMenu1,
                subMenu2,
                itemInSubMenu1,
                itemInSubMenu2
    ...
    private void testAllActionsInMenuBarAreRoughlyCorrect() {
        AppMenuBar menuBar = window.getMainMenuBar();
    ...
    private void testMenuItem(MenuItem menuItem, int depth) {
        if (depth == 0) {
            Assertions.assertTrue(menuItem.getUserData() instanceof AppMainMenuType,
                    menuItem.getClass().getSimpleName() + " '" + menuItem.getText()
                            + "' user data is not an instance of "
                            + AppMainMenuType.class.getSimpleName() + ". Instead it is: "
                            + menuItem.getUserData().getClass().getName());
        } else {
            Assertions.assertTrue(menuItem.getUserData() instanceof AppAction,
                    menuItem.getClass().getSimpleName() + " '" + menuItem.getText()
                            + "' user data is not an instance of "
                            + AppAction.class.getSimpleName() + ". Instead it is: "
                            + menuItem.getUserData().getClass().getName());

            AppAction action = (AppAction) menuItem.getUserData();
    ...
    private void testStructureCorrectness() {
        Menu testingMenu = getMenu(AppMainMenuType.TESTING).orElseThrow();
